...
<head>
    ...
    <?php 
        ...
        $query="select * from user inner join allAcc on user.username=allAcc.username where account_type='SD'";
        $result=mysqli_query($db,$query) or die("login couldnt be completed!!");
        $r=mysqli_query($db,$q);
        $query1="select count(user.username) as total from user inner join allAcc on user.username=allAcc.username where account_type='SD'";
